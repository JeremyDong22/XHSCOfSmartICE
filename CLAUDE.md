...
uv venv && uv pip install playwright fastapi uvicorn google-generativeai python-dotenv pillow
...
5. `DataCleaningService` filters posts by metrics and uses `GeminiLabeler` to classify by image/text
6. `BrowserEventManager` broadcasts state changes to frontend via SSE
7. On shutdown: graceful close with 5s timeout, then force-kill orphaned Chrome processes
